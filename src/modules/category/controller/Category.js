...
//     // If category not found in both locations, return 404
//     if (!category) {
//         return next(new Error(`Category not found ${req.params.categoryId}`, { cause: 404 }));
//     }

//     category.name = category.name[`${locale}`] || `coupons- ${locale}`
//     category.description = category.description[`${locale}`] || `coupons- ${locale}`
...
//     if (!category) {
//         return next(new Error(`Invalid category ID`, { status: 400 }));
//     }
...
//             return next(new Error(`Cannot update category with the same old name`, { status: 409 }));
//         }

//         if (await categoryModel.findOne({ name: req.body.name })) {
//             return next(new Error(`Duplicate category name ${req.body.name}`, { status: 409 }));
//         }
...
//     category.location.push(req.body.location);
